...
export const getLecturerRoute ={
    ...
    handler: async (req, h) => {
        ...
        const {results} = await db.query(
            'select distinct module.module_id, module.name,lecturer.name as lecturer, semester.sem_type as semester, semester.start_date, semester.end_date from lam inner join (module, lecturer, semester, program_modules) on (lam.acc_id = ? and lam.school_id = module.lecturer_id and module.lecturer_id = lecturer.staff_id and module.module_id = program_modules.module_id and program_modules.sem_id = semester.sem_id) order by start_date;', [userId]
        );
